...
abstract class GenerationLayer<T>(protected val seed: Long, protected val parent: GenerationLayer<T>? = null) {
    companion object {
    ...
    abstract fun generate(x: Int, y: Int, width: Int, height: Int): Array<T>

    fun sampleParent(x: Int, y: Int, width: Int, height: Int): Array<T> {
        return this.parent?.generate(x, y, width, height) ?: throw IllegalStateException("could not sample from null parent")
    }
